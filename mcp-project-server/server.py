...
def _run_cmd(cmd: list[str], cwd: Path | None = None, timeout: int = 30) -> str:
    ...
    En Windows, al ocurrir TimeoutExpired se mata el árbol completo de procesos
    con taskkill antes de retornar, evitando que subprocesos nietos (ej. curl)
    mantengan el pipe abierto y cuelguen el servidor MCP indefinidamente.
    """
    proc = None
    try:
        ...
        if proc is not None:
            if sys.platform == "win32":
                # Matar árbol completo para evitar que subprocesos nietos cuelguen el pipe
                subprocess.run(
...
TOOLS = {
    ...
    "log_change": Tool(
        name="log_change",
        description=(
            "Registra un cambio realizado en el historial del agente. "
            "LLAMAR SIEMPRE al terminar cualquier tarea que modifique archivos. "
            "Tags canónicos: relay, dispatcher, telegram, api, db, prompt, mcp, timeout, auth, "
            "bug, feature, config, timing, session, xml, changelog, monitor, orquestador, cfo, "
            "pmo, tacos-api, tacos-bot, cfo-agent."
        ),
        inputSchema={
            "type": "object",
            "properties": {
                "titulo": {
                    "type": "string",
                    "description": "Título conciso del cambio (ej: 'Fix relay race condition')",
                },
                "desc": {
                    "type": "string",
                    "description": "Descripción detallada: qué cambió y por qué",
                },
                "archivos": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Lista de archivos modificados (rutas relativas al proyecto)",
                },
                "tags": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Tags del vocabulario canónico",
                },
                "origen": {
                    "type": "string",
                    "description": "'user' (instrucción del admin), 'autofix' (PMO detectó solo), 'monitor' (alerta del monitor)",
                    "default": "user",
                },
            },
            "required": ["titulo", "desc", "archivos", "tags"],
        },
    ),
    "search_changes": Tool(
        name="search_changes",
        description=(
            "Busca en el historial de cambios del ecosistema. "
            "PMO puede buscar en todos los agentes; otros agentes solo ven sus propios cambios. "
            "Útil cuando el usuario referencia 'ese fix', 'el cambio del relay', etc."
        ),
        inputSchema={
            "type": "object",
            "properties": {
                "query": {
                    "type": "string",
                    "description": "Texto libre a buscar en título y descripción",
                    "default": "",
                },
                "tags": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Filtrar por tags (OR — cualquiera de los tags)",
                    "default": [],
                },
                "agente": {
                    "type": "string",
                    "description": "Filtrar por agente específico (solo PMO puede ver otros agentes)",
                    "default": "",
                },
                "limit": {
                    "type": "integer",
...
CHANGELOG_DIR = Path("C:/SesionBot/changelogs")


async def handle_log_change(args: dict) -> str:
    import json
    ...
    entry = {
        ...
        "agente":  PM2_NAME or PROJECT_NAME,
        "origen":  origen,
        "titulo":  titulo,
        "desc":    desc,
        "archivos": archivos,
    ...
    agent_key = (PM2_NAME or PROJECT_NAME).replace(" ", "-")
    changelog_path = CHANGELOG_DIR / f"{agent_key}.jsonl"
    with open(changelog_path, "a", encoding="utf-8") as f:
        f.write(json.dumps(entry, ensure_ascii=False) + "\n")

    return f"✅ Cambio registrado: {titulo}"


async def handle_search_changes(args: dict) -> str:
    import json
    query  = args.get("query", "").lower()
    tags   = [t.lower() for t in args.get("tags", [])]
    agente = args.get("agente", "").lower()
    limit  = min(int(args.get("limit", 20)), 100)

    agent_key = (PM2_NAME or PROJECT_NAME).replace(" ", "-")
    is_pmo = "pmo" in agent_key.lower()

    if is_pmo:
        files = list(CHANGELOG_DIR.glob("*.jsonl")) if CHANGELOG_DIR.exists() else []
    else:
        f = CHANGELOG_DIR / f"{agent_key}.jsonl"
    ...
    for fp in files:
        ...
        for line in fp.read_text(encoding="utf-8", errors="replace").splitlines():
            ...
            try:
                ...
                continue
            if agente and entry.get("agente", "").lower() != agente:
                continue
            if tags and not any(t in [x.lower() for x in entry.get("tags", [])] for t in tags):
                continue
            searchable = (entry.get("titulo", "") + " " + entry.get("desc", "")).lower()
    ...
    for e in results:
        lines.append(
            f"[{e.get('ts','')}] {e.get('agente','')} ({e.get('origen','')})\n"
            f"  {e.get('titulo','')}\n"
            f"  {e.get('desc','')}\n"
            f"  Archivos: {', '.join(e.get('archivos', []))}\n"
...
async def handle_run_command(args: dict) -> str:
    ...
    # En Windows, shell=True en _run_cmd ya corre el comando via cmd.exe.
    # Pasar como string evita la lentitud de inicializar Git Bash como proceso intermedio.
    if sys.platform == "win32":
        cmd = command  # shell=True lo interpreta cmd.exe directamente
    else:
        cmd = ["bash", "-c", command]
...
HANDLERS = {
    ...
    "search_changes": handle_search_changes,
}
